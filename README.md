...
- **Optional FCM push dispatch:** gRPC **`PushService`** client to the Go **push-worker** in **`many_faces_push`**; device registration via **`POST /api/me/push-token`**; operator smoke via **`POST /api/admin/push/test-self`**. See [`docs/guides/push-notifications-local-dev.md`](../../docs/guides/push-notifications-local-dev.md).
